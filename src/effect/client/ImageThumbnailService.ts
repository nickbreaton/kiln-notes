...
export class ImageThumbnailServiceError
  extends Schema.TaggedError<ImageThumbnailServiceError>()("ImageThumbnailServiceError", {
...
const toBlob = (canvas: HTMLCanvasElement, quality: number) =>
  Effect.tryPromise({
    ...
    catch: (cause) => new ImageThumbnailServiceError({ cause }),
  });

export class ImageThumbnailService extends Effect.Service<ImageThumbnailService>()("ImageThumbnailService", {
  dependencies: [],
  effect: Effect.succeed({
    createThumbnail: (file: File) =>
      Effect.gen(function*() {
        ...
        if (!context) {
          bitmap.close();
          return yield* new ImageThumbnailServiceError({
        ...
        const blob = yield* toBlob(canvas, THUMBNAIL_QUALITY);
