...
export const PieceSlot = ({ variant, status = "drying" }: PieceSlotProps) => {
  if (variant === "target") {
    return (
      <div
        className={`h-[110px] w-[110px] rounded-xl border-2 ${borderByStatus[status]} ${bgByStatus[status]}`}
      />
    );
  }

  return <div className="h-[110px] w-[110px] rounded-xl border-2 border-cream-200 bg-transparent" />;
};
